...
export default function Lobby({ onJoinRoom, onCreateRoom, publicRooms, connectionStatus }: LobbyProps) {
            ...
            <div className="mt-3 flex items-center justify-center gap-2 text-sm">
              ...
              {connectionStatus === 'disconnected' && (
                <>
                  ...
                  <span className="text-red-400">Disconnected - Check network</span>
                </>
              )}
            </div>
          </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl font-bold text-lg shadow-lg transition-all transform ${
                connectionStatus !== 'connected'
                  ...
                  : mode === 'create'
                  ? 'bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 hover:shadow-xl hover:-translate-y-0.5'
                  : 'bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 hover:shadow-xl hover:-translate-y-0.5'
              }`}
            >
              {connectionStatus === 'connecting' && '⏳ Connecting...'}
              {connectionStatus === 'disconnected' && '❌ No Connection'}
              {connectionStatus === 'connected' && (mode === 'create' ? '🚀 Create Room' : '👋 Join Room')}
            </button>
          </form>

          {connectionStatus === 'disconnected' && (
            <div className="mt-4 p-3 bg-red-500/10 border border-red-500/30 rounded-xl text-center">
              <p className="text-red-400 text-sm font-medium">⚠️ Connection Failed</p>
              <p className="text-red-300 text-xs mt-1">Make sure the backend server is running</p>
              <p className="text-gray-400 text-xs mt-1">Check console for details (F12)</p>
            </div>
          )}
