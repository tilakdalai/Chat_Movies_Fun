...
export default function MoviePlayer({ roomCode, movieUrl }: MoviePlayerProps) {
  ...
  const suppressEmitRef = useRef<boolean>(false); // Prevent re-broadcast when applying remote state
  ...
  useEffect(() => {
    ...
    // Request current movie state when component mounts or URL changes
    const socket = socketService.getSocket();
    if (socket && movieUrl) {
      setTimeout(() => {
        socket.emit('request-movie-state', { roomCode });
      }, 300);
    }
  }, [movieUrl, roomCode]);
  ...
  useEffect(() => {
    ...
    const handleMovieStateUpdate = (state: any) => {
      if (!videoRef.current || !playbackUrl) return;

      console.log('Received movie state:', state, 'isSyncing:', isSyncingRef.current);

      // Stop emitting while applying remote state to avoid echo loops
      suppressEmitRef.current = true;

      // Always sync play/pause state immediately
      if (state.isPlaying && videoRef.current.paused) {
        console.log('▶️ Remote PLAY detected - starting playback');
        videoRef.current.play().catch((err) => {
          console.log('Auto-play blocked by browser:', err);
        });
      } else if (!state.isPlaying && !videoRef.current.paused) {
        console.log('⏸️ Remote PAUSE detected - pausing playback');
      ...
      if (!isSyncingRef.current) {
        ...
        // Sync if time difference is more than 0.3 seconds for tighter sync
        if (timeDiff > 0.3) {
          console.log(`⏩ Syncing time: local=${videoRef.current.currentTime.toFixed(2)}s → remote=${state.currentTime.toFixed(2)}s (diff: ${timeDiff.toFixed(2)}s)`);
      ...
      // Allow outgoing emits again after a short delay
      setTimeout(() => {
        suppressEmitRef.current = false;
      }, 250);
    };

    socket.on('movie-state-updated', handleMovieStateUpdate);

    return () => {
      socket.off('movie-state-updated', handleMovieStateUpdate);
    };
  }, [isYouTube, isGoogleDrive, playbackUrl, roomCode]);

  const emitMovieState = useCallback(
    (overrides?: Partial<{ isPlaying: boolean; currentTime: number }>, force = false) => {
      const socket = socketService.getSocket();
      const video = videoRef.current;
      if (!socket || !video || isYouTube || isGoogleDrive || isSyncingRef.current || suppressEmitRef.current) return;

      const now = Date.now();
      // Reduce debounce from 400ms to 200ms for faster sync
      if (!force && now - lastSyncRef.current < 200) {
        return;
      }
  ...
  const handlePlay = () => {
    if (!videoRef.current || isYouTube || isGoogleDrive || suppressEmitRef.current) return;
    console.log('▶️ Local PLAY - broadcasting to room');
    emitMovieState({ isPlaying: true, currentTime: videoRef.current.currentTime }, true);
  };

  const handlePause = () => {
    if (!videoRef.current || isYouTube || isGoogleDrive || suppressEmitRef.current) return;
    console.log('⏸️ Local PAUSE - broadcasting to room');
    emitMovieState({ isPlaying: false, currentTime: videoRef.current.currentTime }, true);
  };

  const handleSeek = () => {
    if (!videoRef.current || isYouTube || isGoogleDrive || suppressEmitRef.current) return;
    console.log('⏩ Local SEEK to', videoRef.current.currentTime.toFixed(2), '- broadcasting to room');
    emitMovieState({ currentTime: videoRef.current.currentTime, isPlaying: !videoRef.current.paused }, true);
  };

  const handleTimeUpdate = () => {
    if (!videoRef.current || videoRef.current.paused || isYouTube || isGoogleDrive || suppressEmitRef.current) return;
    emitMovieState();
  };
