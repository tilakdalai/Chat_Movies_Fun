...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      console.log('Socket connected, fetching public rooms...');
      // Fetch public rooms after connection
      socket.emit('get-public-rooms', {}, (response: { rooms: PublicRoomSummary[] }) => {
        if (response?.rooms) {
          console.log('Received public rooms:', response.rooms);
    ...
    // Only fetch if already connected
    if (socket.connected) {
      socket.emit('get-public-rooms', {}, (response: { rooms: PublicRoomSummary[] }) => {
        if (response?.rooms) {
          setPublicRooms(response.rooms);
  ...
  const handleJoinRoom = (code: string, displayName: string, newUserId?: string) => {
    ...
    socket.emit('join-room', { roomCode: code, displayName, userId: newUserId }, (response: any) => {
      ...
      if (response.success) {
        ...
        alert(response.error || 'Failed to join room. The room may not exist.');
      }
    });
  };
  ...
  const handleCreateRoom = (displayName: string, visibility: 'public' | 'private') => {
    ...
    console.log('Creating room with visibility:', visibility);
