...
export class ChessGame {
    initializeGame(whitePlayerId, blackPlayerId) {
    ...
    makeMove(gameState, playerId, from, to, promotion) {
        // Validate player
        ...
        if (isWhite && gameState.sideToMove !== 'white')
            return { success: false, error: 'Not your turn' };
        if (isBlack && gameState.sideToMove !== 'black')
            return { success: false, error: 'Not your turn' };
        // Validate move
        const legalMoves = this.getLegalMoves(gameState);
        const move = legalMoves.find(m => m.from.x === from.x && m.from.y === from.y &&
    ...
    generatePawnMoves(state, from, piece, moves) {
    ...
    generateKingMoves(state, from, piece, moves) {
        const offsets = [
    ...
    generateSlidingMoves(state, from, piece, moves) {
        const directions = [];
    ...
    generateStepMoves(state, from, piece, moves, offsets) {
    ...
    applyMove(state, move, testMode = false) {
        // Deep copy state
    ...
    isKingInCheck(state, color) {
        ...
        if (!kingPos)
            return true; // Should not happen unless king captured (illegal)
        return this.isSquareAttacked(state, kingPos, this.getOpponentColor(color));
    }
    isSquareAttacked(state, pos, attackerColor) {
            ...
            for (let i = 1; i < 8; i++) {
                ...
                if (p) {
                    if (p.color === attackerColor) {
                        ...
                        if (p.type === 'queen')
                            return true;
                        if (isDiagonal && p.type === 'bishop')
                            return true;
                        if (!isDiagonal && p.type === 'rook')
                            return true;
                    }
                    break; // Blocked
                }
        ...
        for (const off of kingOffsets) {
            ...
            if (this.isValidPos(t)) {
                const p = state.board[t.y][t.x];
                if (p && p.color === attackerColor && p.type === 'king')
                    return true;
            }
        }
        return false;
    ...
    isThreefoldRepetition(state) {
        ...
        const getPosition = (fen) => fen.split(' ').slice(0, 4).join(' ');
