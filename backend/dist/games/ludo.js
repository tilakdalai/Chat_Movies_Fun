...
export class LudoGame {
    constructor() {
        this.BOARD_SIZE = 52;
        this.HOME_SIZE = 6;
        this.SAFE_INDICES = [0, 8, 13, 21, 26, 34, 39, 47]; // 0-based indices of safe spots (Start squares + Stars)
    ...
    initializeGame(playerIds) {
        ...
        const players = playerIds.slice(0, 4).map((userId, index) => ({
            userId,
            color: colors[index],
    ...
    handleDiceRoll(gameState, playerId) {
    ...
    moveToken(gameState, playerId, tokenId) {
        const player = gameState.players[gameState.currentPlayerIndex];
        if (player.userId !== playerId) {
            return { success: false, error: 'Not your turn' };
        }
    ...
    stepsToHomeEntry(currentIndex, color) {
