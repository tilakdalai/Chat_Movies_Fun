...
import fs from 'fs';
import { streamTorrent } from './torrentWorker.js';
const app = express();
...
function getPublicUnoState(gameState) {
    return {
        ...gameState,
...
app.get('/api/torrent-play', async (req, res) => {
    try {
        ...
        await streamTorrent(magnet, req, res);
    }
    catch (error) {
        if (!res.headersSent) {
...
io.on('connection', (socket) => {
    ...
    socket.on('get-public-rooms', (payload, callback) => {
    ...
    // Movie events
    socket.on('set-movie-url', (data) => {
        const room = roomManager.getRoom(data.roomCode);
        if (room) {
            room.movieUrl = data.url;
            // Reset state when new URL is set
            room.movieState = {
                isPlaying: false,
                currentTime: 0,
                lastUpdated: Date.now()
            };
            // First emit URL, then state
            io.to(data.roomCode).emit('movie-url-updated', data.url);
            // Give a brief moment for video to load, then send initial state
            setTimeout(() => {
                io.to(data.roomCode).emit('movie-state-updated', room.movieState);
            }, 100);
        }
    });
    socket.on('update-movie-state', (data) => {
        const room = roomManager.getRoom(data.roomCode);
        if (room) {
            room.movieState = {
                ...data.state,
            ...
            io.to(data.roomCode).emit('movie-state-updated', room.movieState);
            console.log(`Movie state broadcasted to room ${data.roomCode}:`, room.movieState);
        }
    });
    // Send current movie state when requested (for new joiners)
    socket.on('request-movie-state', (data) => {
        const room = roomManager.getRoom(data.roomCode);
        if (room && room.movieState) {
            socket.emit('movie-state-updated', room.movieState);
            console.log(`Sent current movie state to new joiner in room ${data.roomCode}:`, room.movieState);
        }
    });
    socket.on('add-uploaded-movie', (data) => {
    ...
    socket.on('remove-uploaded-movie', (data) => {
    ...
    socket.on('add-to-queue', (data, callback) => {
        try {
    ...
    socket.on('remove-from-queue', (data, callback) => {
        try {
            ...
            if (success) {
                ...
                if (callback)
                    callback({ success: false, error: 'Failed to remove from queue' });
            }
        }
        catch (error) {
            if (callback)
    ...
    socket.on('set-current-track', (data) => {
    ...
    socket.on('update-music-state', (data) => {
        const room = roomManager.getRoom(data.roomCode);
        if (room) {
    ...
    socket.on('search-youtube', async (query, callback) => {
        try {
    ...
    // Uno game actions
    socket.on('uno-play-card', (data, callback) => {
        try {
            const room = roomManager.getRoom(data.roomCode);
    ...
    socket.on('uno-draw-card', (data, callback) => {
        try {
            const room = roomManager.getRoom(data.roomCode);
            if (!room || room.currentGame !== 'uno') {
                callback({ success: false, error: 'Invalid game state' });
    ...
    // Ludo game actions
    socket.on('ludo-roll-dice', (data, callback) => {
        try {
            const room = roomManager.getRoom(data.roomCode);
    ...
    socket.on('ludo-move-token', (data, callback) => {
        try {
            const room = roomManager.getRoom(data.roomCode);
            if (!room || room.currentGame !== 'ludo') {
                callback({ success: false, error: 'Invalid game state' });
    ...
    // Chess game actions
    socket.on('chess-move', (data, callback) => {
        try {
            const room = roomManager.getRoom(data.roomCode);
            ...
            const result = chessGame.makeMove(room.gameState, data.userId, fromCoord, toCoord, data.promotion);
            if (result.success) {
    ...
    // Catch UNO failure
    socket.on('uno-catch-failure', (data, callback) => {
        try {
            const room = roomManager.getRoom(data.roomCode);
            if (!room || room.currentGame !== 'uno') {
                if (callback)
